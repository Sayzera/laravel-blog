          ...
          <div class="col-md-12">
            <div class="block text-center">
              ...
              <ul class="list-inline">
                ...
                <li class="list-inline-item"><a href="#" class="text-white-50">Detay</a></li>
              </ul>
            </div>
          </div>
        </div>
    ...
    <section class="section blog-wrap bg-gray">
        ...
        <div class="col-lg-12 mb-5">
            <div class="single-blog-item">
                <img src="{{asset('images/blog/'.$result['blog_single']->resim.' ')}}" alt="" class="img-fluid rounded">

                <div class="blog-item-content bg-white p-5">
                    <div class="blog-item-meta bg-gray py-1 px-2">
                        <span class="text-muted text-capitalize mr-3"><i class="ti-pencil-alt mr-2"></i>{{$result['blog_single']->konu}}</span>
                        {{-- <span class="text-muted text-capitalize mr-3"><i class="ti-comment mr-2"></i>5 Comments</span> --}}
                        <span class="text-black text-capitalize mr-3"><i class="ti-time mr-1"></i>{{$result['blog_single']->tarih}}</span>
                    </div>

                    <h2 class="mt-3 mb-4"><a href="blog-single.html">{{$result['blog_single']->konu_basligi}}</a></h2>
                    <p class="lead mb-4">
                        <?=$result['blog_single']->content?>
                    </p>

                    <div class="tag-option mt-5 clearfix">
                        <ul class="float-left list-inline">
                            <li>Tags:</li>
                            @foreach (explode(',',$result['blog_single']->etiketler) as $item)
                            <li class="list-inline-item"><a href="#" rel="tag">{{$item}}</a></li>
                            @endforeach
                           </ul>
        ...
        <div class="sidebar-widget latest-post card border-0 p-4 mb-3">
            <h5>En son gonderilenler</h5>

            @foreach ($result['last3'] as $item)
            <div class="media border-bottom py-3">
                <a href="{{route('blog.detay',['blogid'=> $item->id])}}"><img class="mr-4" width="100" height="100" src="{{asset('images/blog/'.$item->resim.'')}}" alt=""></a>
                <div class="media-body">
                    <h6 class="my-2"><a href="{{route('blog.detay',['blogid'=> $item->id])}}"">{{$item->konu_basligi}}</a></h6>
                    <span class="text-sm text-muted">{{$item->tarih}}</span>
                </div>
            </div>
